...
// = > compiler generated functions equivalent to :
...
public:
    ...
    dogGen operator=(const dogGen& rhs) {} // Member by member copying
...
class dogC {
    collar m_collar;
    string m_name; // Using a regular member variable instead of a reference

public:
    dogC(string name = "DefaultName") : m_name(name) {}
};

int main(int argc, char* argv[]) {
    dogC dogc("Henry"); // Creating a dogC object with a specified name
    return 0;
}
...
1. Default constructor	if no other constructors does nothing
4. Destructor	        if no destructor does nothing
2. Copy constructor	    if no move constructor and no move assignment copies all members
3. Copy assignment	    if no move constructor and no move assignment copies all members
5. Move constructor	    if no destructor, no copy constructor and no copy nor move assignment
moves all members
6. Move assignment	    if no destructor, no copy constructor and no copy nor move assignment
moves all members
...
class Dog { // 1, 2, 3, 4, 5, 6
    ...
    Dog operator=(const Dog&); // copy assignment operator: member by member copying
...
class Fish {   // 1, 2, 3 // 2 and 3 are deprecated
    ~Fish() {} // destructor
};
